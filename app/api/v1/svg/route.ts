...
export async function GET(request: NextRequest) {
  ...
  if (!vector) {
    return NextResponse.json(
      { error: "Missing required parameter", detail: "The 'vector' query parameter is required" },
      { status: 400 }
    );
  }
  ...
  if (!vector.startsWith("CVSS:")) {
    return NextResponse.json(
      { error: "Invalid CVSS vector", detail: "Vector must start with 'CVSS:'" },
      { status: 400 }
    );
  }
  ...
  if (isNaN(size) || size < 16 || size > 1024) {
    return NextResponse.json(
      { error: "Invalid size", detail: "Size must be a number between 16 and 1024" },
      { status: 400 }
    );
  }
  ...
  if (score !== undefined && (isNaN(score) || score < 0 || score > 10)) {
    return NextResponse.json(
      { error: "Invalid score", detail: "Score must be a number between 0 and 10" },
      { status: 400 }
    );
  }
